...
const server = process.env.FebecaHost;
const schema = process.env.FebecaSchema;
const user = process.env.FebecaUser;
const password = process.env.FebecaPassword;
                            ...
                            if (err) {
                                ...
                                statement.setFetchSize(100, function (err) {
                                    if (err) {
                                        ...
                                        statement.executeQuery(`SELECT * FROM BECOBVAL.VENTASP WHERE FACTVP LIKE '%${factNum}%'`,
                            ...
                            if (err) {
                                ...
                                statement.setFetchSize(100, function (err) {
                                    if (err) {
                                        ...
                                        statement.executeQuery(`SELECT * FROM SICADMEFB.VENTASP WHERE FACTVP LIKE '%${factNum}%'`,
                                            function (err, resultset) {
                                                if (err) {
                                                    console.log(err)
                                                    res.json({err});
                            ...
                            if (err) {
                                ...
                                statement.setFetchSize(100, function (err) {
                                    if (err) {
                                        ...
                                        statement.executeQuery(`SELECT NAME, EXT_EMAIL FROM ${process.env.FebecaSchema}.C_BPARTNER WHERE VALUE LIKE '${clientKey}'`,
                                            function (err, resultset) {
                                                if (err) {
                                                    console.log(err)
                                                    res.json({err});
                            ...
                            if (err) {
                                ...
                                statement.setFetchSize(100, function (err) {
                                    if (err) {
                                        ...
                                        statement.executeQuery("SELECT  VP.FACTVP AS FACTURA, BP.NAME AS CLIENTE, VP.VENDVP AS ZONA, U.NAME, U.EMAIL AS VENDEDOR FROM BECOBVAL.VENTASP VP JOIN "+process.env.FebecaSchema+".C_BPARTNER BP ON DIGITS(VP.CLI1VP)||DIGITS(VP.CLI2VP)||DIGITS(VP.CLI3VP)=BP.VALUE LEFT JOIN "+process.env.FebecaSchema+".C_SALESREGION SR ON CAST(VP.VENDVP AS VARCHAR(40))=SR.VALUE LEFT JOIN "+process.env.FebecaSchema+".AD_USER U ON SR.SALESREP_ID=U.AD_USER_ID WHERE VP.FACTVP=SUBSTRING('"+factNum+"', 3, 27) ",
                                            function (err, resultset) {
                                                if (err) {
                                                    console.log(err)
                                                    res.json({err});
...
module.exports = { febeca: { getFactByNumBs, getFactByNumUsd, getEmail, getSellerEmail } };